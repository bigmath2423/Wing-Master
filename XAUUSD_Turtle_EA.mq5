...
#property strict
#property description "Trend Following EA for XAUUSD - Turtle style breakout"
#property description "EMA200 trend filter + Donchian breakout + ADX + ATR risk sizing"
...
input ENUM_TIMEFRAMES TradeTimeframe   = PERIOD_H1;    // Timeframe de trading
...
int      hEMA  = INVALID_HANDLE;   // handle iMA
int      hADX  = INVALID_HANDLE;   // handle iADX
int      hATR  = INVALID_HANDLE;   // handle iATR
...
int OnInit()
{
   ...
   hEMA = iMA(_Symbol, TradeTimeframe, EMA_Period, 0, MODE_EMA, PRICE_CLOSE);
   ...
   PrintFormat("INIT OK | Symbole=%s | TF=%s | EMA=%d | Breakout=%d | ADX=%d(min %.1f) | ATR=%d x%.1f | RR=%.1f | Risk=%.2f%% | Magic=%I64u",
               _Symbol, EnumToString(TradeTimeframe), EMA_Period, Breakout_Period,
               ADX_Period, ADX_Min, ATR_Period, ATR_Multiplier, Reward_Ratio,
...
void OnTick()
{
   ...
   // --- Conditions Turtle ---
   bool trendUp   = (lastClose > ema);
   bool trendDown = (lastClose < ema);
