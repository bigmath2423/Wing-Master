...
t['ax']=adxr.rolling(500,min_periods=200).rank(pct=True).reindex(t.t_in).values
...
THR=0.50
mk=(t.ax<=THR).values
print(f"AVANT / APRES — filtre ADXpct <= {THR} (2022-2026, moteur Python, cout 0.20 $/oz)\n")
b=line("AVANT (v11 actuelle)",t.R.values)
a2=line(f"APRES (ADXpct<={THR})",t.R.values[mk])
line(f"  [rejete : ADXpct>{THR}]",t.R.values[~mk])
...
for i,ii in enumerate(idx):
    s=t.iloc[ii]; mm=(s.ax<=THR).values; r=s.R.values
...
for nm,sl in [('apprentissage',slice(0,half)),('validation   ',slice(half,None))]:
    s=t.iloc[sl]; mm=(s.ax<=THR).values; r=s.R.values
